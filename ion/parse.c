...
parse_expr_compound_field()
{
    if (match_token(TOKEN_LBRACKET)) {
        ...
        return (CompoundField) {
            FIELD_INDEX, parse_expr(), .index = index
        };
    } else {
...
Expr*
parse_expr_compound(Typespec* type)
{
...
parse_stmt_if(void)
{
    ...
    while (match_keyword(else_keyword)) {
        ...
        buf_push(elseifs, (ElseIf) {
            elseif_cond, elseif_block
        });
    }
...
parse_stmt_switch_case(void)
{
    ...
    return (SwitchCase) {
        exprs, buf_len(exprs), is_default, stmt_list(stmts, buf_len(stmts))
    };
}

Stmt*
parse_stmt_switch(void)
{
...
AggregateItem
parse_decl_aggregate_item(void)
{
    ...
    return (AggregateItem) {
        names, buf_len(names), type
    };
}

Decl*
parse_decl_aggregate(DeclKind kind)
{
