...
type_ptr(Type* elem)
{
    ...
    type->ptr.elem = elem;
    buf_push(cached_ptr_types, (CachedPtrType) {
        elem, type
    });
    return type;
}
...
Type*
type_array(Type* elem, size_t size)
{
    ...
    buf_push(cached_array_types, (CachedArrayType) {
        elem, size, type
    });
    return type;
}
...
Type*
type_func(Type** params, size_t num_params, Type* ret)
{
    ...
    type->func.ret = ret;
    buf_push(cached_func_types, (CachedFuncType) {
        params, num_params, ret, type
    });
    return type;
}
...
bool
duplicate_fields(TypeField* fields, size_t num_fields)
{
...
void
type_complete_struct(Type* type, TypeField* fields, size_t num_fields)
{
    assert(type->kind == TYPE_COMPLETING);
...
void
type_complete_union(Type* type, TypeField* fields, size_t num_fields)
{
    assert(type->kind == TYPE_COMPLETING);
...
Sym*
sym_new(SymKind kind, const char* name, Decl* decl)
{
...
Sym*
sym_enum_const(const char* name, Decl* decl)
{
...
void
sym_push_var(const char* name, Type* type)
{
...
Sym*
sym_global_type(const char* name, Type* type)
{
...
complete_type(Type* type)
{
    ...
    for (size_t i = 0; i < decl->aggregate.num_items; i++) {
        ...
        for (size_t j = 0; j < item.num_names; j++) {
            buf_push(fields, (TypeField) {
                item.names[j], item_type
            });
        }
    }
...
Type*
resolve_decl_const(Decl* decl, int64_t* val)
{
...
void
resolve_stmt_block(StmtList block, Type* ret_type)
{
    Sym* start = sym_enter();
...
int64_t
eval_int_unary(TokenKind op, int64_t val)
{
    switch (op) {
    case TOKEN_ADD:
        return +val;
...
int64_t
eval_int_binary(TokenKind op, int64_t left, int64_t right)
{
    switch (op) {
    case TOKEN_MUL:
        return left * right;
...
size_t
aggregate_field_index(Type* type, const char* name)
{
